...
for ($i = $start; $i < $end; $i++) {
    $sql = "SELECT goods_id, provider_name FROM ecs_goods WHERE brand_id='0' " . 
        " AND goods_id > ${last_id} LIMIT ${limit} ";
...
function get_brand_like($brand_name = '') {
    if (!empty($brand_name)) {
        ...
        $return = array();
        $brands = array();
        $sql = "SELECT brand_id FROM ecs_brand WHERE brand_name like '$frist_word%'";
        $result = mysql_query($sql);
